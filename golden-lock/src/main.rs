...
fn main() {
    ...
    let (results, computed_root) = lock::evaluate_vectors(&vectors);
    ...
    match cmd.as_str() {
        "generate" => {
            ...
            cmd_generate(&results, &computed_root, &baseline_path);
        }
        "verify" => {
            ...
            cmd_verify(&results, &computed_root, baseline_path);
        }
        other => {
...
fn cmd_generate(results: &[lock::VectorResult], global: &str, baseline_path: &str) {
...
fn cmd_verify(results: &[lock::VectorResult], computed: &str, baseline_path: &str) {
