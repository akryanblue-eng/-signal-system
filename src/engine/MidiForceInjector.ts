...
export class FlowFieldInjector {
  ...
  apply(event: MidiForceEvent | null): void {
    if (!event) return;
    if (event.type === 'chaos'   && event.intensity !== undefined) this.chaos   = event.intensity;
    if (event.type === 'damping' && event.stability !== undefined) this.damping = event.stability;
