...
use signal_system::director_loop_v2::{
    CorrectionActionV2, CorrectionRuleV2, CorrectionV2, RegenEventV2, RegenTrigger, RunStatus,
    compute_input_hash, compute_output_hash,
};
use signal_system::verifier_v2::{AuditStatusV2, FailureCodeV2, GateResultV2, verify_v2, verify_bytes_v2};
...
fn v2_d01_input_hash_drift() {
    ...
    assert_eq!(r.gates.input_hash.failure_code,      Some(FailureCodeV2::InputHashDrift));
...
fn v2_d02_output_hash_drift() {
    ...
    assert_eq!(r.gates.output_hash.failure_code,     Some(FailureCodeV2::OutputHashDrift));
}
...
fn v2_d03_schema_violation() {
    let raw = fs::read_to_string("fixtures/v2/temporal_collapse_001/director_loop_run_v2.json").unwrap();
    let mut val: serde_json::Value = serde_json::from_str(&raw).unwrap();
    val.as_object_mut()
        .unwrap()
        .insert("unknown_field".into(), serde_json::Value::String("x".into()));
    let r = verify_bytes_v2(&serde_json::to_vec(&val).unwrap());
    assert_eq!(r.audit_status,                       AuditStatusV2::Fail);
    assert_eq!(r.gates.schema.result,                GateResultV2::Fail);
    assert_eq!(r.gates.schema.failure_code,          Some(FailureCodeV2::SchemaViolation));
...
fn v2_d04_timeline_empty() {
    ...
    assert_eq!(r.gates.structural.failure_code,      Some(FailureCodeV2::TimelineEmpty));
    assert_eq!(r.gates.state.result,                 GateResultV2::NotEvaluated);
    assert_eq!(r.gates.input_hash.result,            GateResultV2::NotEvaluated);
    assert_eq!(r.gates.output_hash.result,           GateResultV2::NotEvaluated);
}
...
fn v2_d05_timeline_non_monotonic() {
    ...
    assert_eq!(r.audit_status,                       AuditStatusV2::Fail);
    assert_eq!(r.gates.structural.result,            GateResultV2::Fail);
    assert_eq!(r.gates.structural.failure_code,      Some(FailureCodeV2::TimelineNonMonotonic));
    assert_eq!(r.gates.state.result,                 GateResultV2::NotEvaluated);
    assert_eq!(r.gates.input_hash.result,            GateResultV2::NotEvaluated);
}
...
fn v2_d06_timeline_status_mismatch() {
    ...
    assert_eq!(r.audit_status,                       AuditStatusV2::Fail);
    assert_eq!(r.gates.structural.result,            GateResultV2::Fail);
    assert_eq!(r.gates.structural.failure_code,      Some(FailureCodeV2::TimelineStatusMismatch));
    assert_eq!(r.gates.state.result,                 GateResultV2::NotEvaluated);
    assert_eq!(r.gates.input_hash.result,            GateResultV2::NotEvaluated);
}
...
fn v2_d07_corrections_orphaned_beat_id() {
    ...
    assert_eq!(r.audit_status,                       AuditStatusV2::Fail);
    assert_eq!(r.gates.structural.result,            GateResultV2::Fail);
    assert_eq!(r.gates.structural.failure_code,      Some(FailureCodeV2::CorrectionsOrphanedBeatId));
    assert_eq!(r.gates.state.result,                 GateResultV2::NotEvaluated);
    assert_eq!(r.gates.input_hash.result,            GateResultV2::NotEvaluated);
}
...
fn v2_d08_audit_notes_out_of_order() {
    ...
    assert_eq!(r.audit_status,                       AuditStatusV2::Fail);
    assert_eq!(r.gates.structural.result,            GateResultV2::Fail);
    assert_eq!(r.gates.structural.failure_code,      Some(FailureCodeV2::AuditNotesOutOfOrder));
    assert_eq!(r.gates.input_hash.result,            GateResultV2::NotEvaluated);
    assert_eq!(r.gates.output_hash.result,           GateResultV2::NotEvaluated);
}
...
fn v2_d09_audit_warnings_out_of_order() {
    ...
    assert_eq!(r.audit_status,                       AuditStatusV2::Fail);
    assert_eq!(r.gates.structural.result,            GateResultV2::Fail);
    assert_eq!(r.gates.structural.failure_code,      Some(FailureCodeV2::AuditWarningsOutOfOrder));
    assert_eq!(r.gates.input_hash.result,            GateResultV2::NotEvaluated);
    assert_eq!(r.gates.output_hash.result,           GateResultV2::NotEvaluated);
}
...
fn v2_d10_passed_below_threshold() {
    ...
    assert_eq!(r.gates.state.failure_code,           Some(FailureCodeV2::PassedBelowThreshold));
    assert_eq!(r.gates.input_hash.result,            GateResultV2::NotEvaluated);
    assert_eq!(r.gates.output_hash.result,           GateResultV2::NotEvaluated);
}
...
fn v2_d11_passed_coherence_regressed() {
    ...
    assert_eq!(r.audit_status,                       AuditStatusV2::Fail);
    assert_eq!(r.gates.state.result,                 GateResultV2::Fail);
    assert_eq!(r.gates.state.failure_code,           Some(FailureCodeV2::PassedCoherenceRegressed));
    assert_eq!(r.gates.input_hash.result,            GateResultV2::NotEvaluated);
}
...
fn v2_d12_regen_without_events() {
    ...
    assert_eq!(r.gates.structural.result,            GateResultV2::Pass);
    assert_eq!(r.gates.state.result,                 GateResultV2::Fail);
    assert_eq!(r.gates.state.failure_code,           Some(FailureCodeV2::RegenWithoutEvents));
    assert_eq!(r.gates.state.path.as_deref(),        Some("execution.regen_events"));
    assert_eq!(r.gates.input_hash.result,            GateResultV2::NotEvaluated);
}
...
fn v2_correction_anchored_by_regen_event_passes() {
    ...
    run.execution.timeline.push(signal_system::director_loop_v2::TimelineStep {
        step:    3,
