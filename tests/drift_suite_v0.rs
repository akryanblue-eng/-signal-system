...
use signal_system::director_loop::{
    compute_input_hash, compute_output_hash, Correction, CorrectionAction, CorrectionRule,
    DirectorLoopRun, Fixture, RegenEvent, Status,
};
use signal_system::verifier::{verify, verify_bytes, AuditStatus, FailureCode, GateResult};
...
fn d01_input_hash_drift() {
    let mut run = load();
    run.fixture = Fixture {
        fixture_id: "temporal_collapse_001_mutated".to_string(),
    };
    let r = verify(&run);
    ...
    assert_eq!(
        ...
        Some(FailureCode::InputHashDrift)
    );
...
fn d02_output_hash_drift() {
    ...
    assert_eq!(
        ...
        Some(FailureCode::OutputHashDrift)
    );
}
...
fn d04_corrections_out_of_order() {
    ...
    assert_eq!(
        r.gates.invariants.failure_code,
        Some(FailureCode::CorrectionsOutOfOrder)
    );
...
fn d05_passed_below_threshold() {
    ...
    assert_eq!(
        r.gates.invariants.failure_code,
        Some(FailureCode::PassedBelowThreshold)
    );
    assert_eq!(r.gates.input_hash.result, GateResult::NotEvaluated);
}
...
fn d06_regen_without_events() {
    ...
    assert_eq!(
        r.gates.invariants.failure_code,
        Some(FailureCode::RegenWithoutEvents)
    );
    assert_eq!(r.gates.input_hash.result, GateResult::NotEvaluated);
}
...
fn d07_canonicalization_equivalence() {
    ...
    let corrections = vec![
        Correction {
            rule: CorrectionRule::RhythmDrift,
            beat_id: "beat_001".into(),
            action: CorrectionAction::AdjustXfade,
        },
        Correction {
            rule: CorrectionRule::AvAlignment,
            beat_id: "beat_003".into(),
            action: CorrectionAction::AdjustXfade,
        },
    ];
...
fn d08_audit_notes_out_of_order() {
    ...
    assert_eq!(
        r.gates.invariants.failure_code,
        Some(FailureCode::AuditNotesOutOfOrder)
    );
    assert_eq!(r.gates.input_hash.result, GateResult::NotEvaluated);
    assert_eq!(r.gates.output_hash.result, GateResult::NotEvaluated);
}
